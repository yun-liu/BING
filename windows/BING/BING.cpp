...
#ifdef TRAIN_SET
	const int Num = _voc.trainNum;
#else
	const int Num = _voc.testNum;
#endif
...
#else
		getObjBndBoxes(imread(format(_S(_voc.imgPathW), _S(_voc.testSet[i]))), boxes[i], numDetPerSize);
#endif
#endif
...
void Objectness::evaluatePerImgRecall(const vector<vector<Vec4i>> &boxesT, CStr &saveName, const int NUM_WIN)
{
	...
	int idx[12] = {1, 10, 100, 300, 500, 650, 800, 1000, 2000, 3000, 4000, 5000};
	for (int i = 0; i < 12; i++){
		if (idx[i] > NUM_WIN)
			continue;
		cout.setf(ios::fixed);
...
bool Objectness::matRead(CStr& filename, Mat& _M){
