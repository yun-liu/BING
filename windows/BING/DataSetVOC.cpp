...
DataSetVOC::DataSetVOC(CStr &_wkDir)
{
	...
	//valSet = CmFile::loadStrList(wkDir + "ImageSets/Main/val.txt");
...
void DataSetVOC::loadAnnotations()
{
	...
	/*gtValBoxes.resize(valNum);
	gtValClsIdx.resize(valNum);
	for (int i = 0; i < valNum; i++)
		if (!loadBBoxes(valSet[i], gtValBoxes[i], gtValClsIdx[i]))
			return;*/
